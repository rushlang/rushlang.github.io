...
- 0性能损失。内置全调用链类型推导，尽可能推导出变量的类型以便加速。大多数情况下，类型推导会生效，这时可以做到编译出来的C++代码和手写的C++代码完全相同的性能。QuickPy的对手只有C++，其它语言运行性能太弱不配做QuickPy的对手。QuickPy是目前运行性能最强的动态类型AOT编译器。
...
##### 性能测试：（i5 8500, win7 x64，双方均使用VS2019的cl.exe /O2编译）

||QuickPy|C++|
|:---:|:---:|:---:|
|递归计算fib|1.824000 s|1856 ms|
|双重循环计算|2.034500 s|1950 ms|
...
##### 不兼容的代码怎么办？
目前QuickPy可以支持文件级部分编译，也就是和CPython混合执行，这样就可以把不兼容的部分扔给CPython执行。未来还打算支持函数级部分编译、自动标注不兼容代码、自动转换不兼容的代码为兼容的代码，这些技术可以略提高兼容性，但仍不可能100%兼容。
